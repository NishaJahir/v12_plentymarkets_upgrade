...
class NovalnetServiceProvider extends ServiceProvider
{
                          ...
                          EventProceduresService $eventProceduresService)
    {
        ...
        $billingAddress = $addressRepository->findAddressById($billingAddressId);
					$paymentDetails = $dataBase->query(TransactionLog::class)->where('paymentName', '=', strtolower($paymentKey))->where('customerEmail', '=', $billingAddress->email)->where('saveOneTimeToken', '!=', null)->where('maskingDetails', '!=', null)->orderBy('id','DESC')->limit(2)->get();
